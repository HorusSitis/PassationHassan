# -*- coding: utf-8 -*-
"""
Created on Wed Jul 18 10:50:24 2018
...
def raffinemment_maillage(r,mesh):
	...
	for f in facets(c):
	    if ((sqrt((f.midpoint()[0]-c1_x)**2+(f.midpoint()[1]-c1_y)**2)<=1.3*r) or (sqrt((f.midpoint()[0]-c2_x)**2+(f.midpoint()[1]-c2_y)**2)<=1.3*r) or (sqrt((f.midpoint()[0]-c3_x)**2+(f.midpoint()[1]-c3_y)**2)<=1.3*r) or (sqrt((f.midpoint()[0]-c4_x)**2+(f.midpoint()[1]-c4_y)**2)<=1.3*r)):
		markers[c] = True
...
repertoire_final="Solutions_homog_interp_complexe_1/"

File("Solutions_homog_interp_complexe_1/mesh_complexe_1.xml.gz") << mesh_fixe
...
#        def inside(self, x, on_boundary):
#            return (on_boundary and between((x[0]-c1_x), (-tol, 2*r+tol)) and between((x[1]-c1_y), (-tol, 2*r+tol)))


#    class Obstacle2(SubDomain):
#        def inside(self, x, on_boundary):
#            return (on_boundary and between((x[0]-c2_x), (-2*r-tol, tol)) and between((x[1]-c2_y), (-tol, 2*r+tol)))


#    class Obstacle3(SubDomain):
#        def inside(self, x, on_boundary):
#            return (on_boundary and between((x[0]-c3_x), (-2*r-tol, tol)) and between((x[1]-c3_y), (-2*r-tol, tol)))


#    class Obstacle4(SubDomain):
#        def inside(self, x, on_boundary):
#            return (on_boundary and between((x[0]-c4_x), (-tol, 2*r+tol)) and between((x[1]-c4_y), (-2*r-tol, tol)))
...
#    print u(0,0.5),u(1,0.5),u(0.5,0),u(0.5,1),u(0,0.4),u(1,0.4),u(0.5,0.5),u(1-r-0.01,0),u(1-r-0.01,1)
#    if r==R[4]:
#        plot(u)
#        plt.show()
...
#    plt.show()
    ...
    fe=1.0
    fcar=Expression(("((sqrt(pow((x[0]),2)+pow((x[1]),2)) - r < 0.0) or (sqrt(pow((x[0]-1),2)+pow((x[1]),2)) - r < 0.0) or (sqrt(pow((x[0]-1),2)+pow((x[1]-1),2)) - r < 0.0) or (sqrt(pow((x[0]),2)+pow((x[1]-1),2)) - r < 0.0)) ? fi : fe", "((sqrt(pow((x[0]),2)+pow((x[1]),2)) - r < 0.0) or (sqrt(pow((x[0]-1),2)+pow((x[1]),2)) - r < 0.0) or (sqrt(pow((x[0]-1),2)+pow((x[1]-1),2)) - r < 0.0) or (sqrt(pow((x[0]),2)+pow((x[1]-1),2)) - r < 0.0)) ? fi : fe"),degree=2, fi=fi, fe=fe, r=r)
    ...
    class Omega_fluide(SubDomain):
        def inside(self, x, on_boundary):
            return (((sqrt((x[0] - c1_x)**2 + (x[1] - c1_y)**2)-r) >= 0.0) and ((sqrt((x[0] - c2_x)**2 + (x[1] - c2_y)**2)-r) >= 0.0) and ((sqrt((x[0] - c3_x)**2 + (x[1] - c3_y)**2)-r) >= 0.0) and ((sqrt((x[0] - c4_x)**2 + (x[1] - c4_y)**2)-r) >= 0.0))
