# -*- coding: utf-8 -*-
"""
Created on Tue Jul 17 11:04:27 2018
...
repertoire_final="Solutions_homog_interp_complexe/"

File("Solutions_homog_interp_complexe/mesh_complexe.xml.gz") << mesh_fixe
...
#        def inside(self, x, on_boundary):
#            return (on_boundary and between((x[0]-c_x), (-2*0.45-tol, 2*0.45+tol)) and between((x[1]-c_x), (-2*0.45-tol, 2*0.45+tol)))
...
#            return (on_boundary and between((x[0]-c4_x), (-tol, 2*r+tol)) and between((x[1]-c4_y), (-2*r-tol, tol)))
    ...
    print u(0.5,0.03),u(0.5,0.97),u(1,0.5),u(0,0.5),u(0.5,0),u(0.5,1)
...
#    plt.show()
    ...
    num_surf=1
    H=assemble(grad(u_fixe1)[0,0]*dx1(num_surf))
    M=assemble(grad(u_fixe1)[1,1]*dx1(num_surf))
    A=assemble(grad(u_fixe1)[0,1]*dx1(num_surf))
    C=assemble(grad(u_fixe1)[1,0]*dx1(num_surf))
    print H,M,A,C
    ep=(1-pi*((r**2)+0.45**2))
    D=((H/ep)+1)*ep
    print ("hi",r,ep,D)

#    print u_fixe1(0.5,0.03),u_fixe1(0.5,0.97),u_fixe1(0.005,0.005),u_fixe1(1,0.5),u_fixe1(0,0.5),u_fixe1(0.5,0),u_fixe1(0.5,1)#,u(0.5,0),u(0.5,1),u(0,0.4),u(1,0.4),u(0.5+0.45,0.5),u(1-r-0.01,0),u(1-r-0.01,1)
