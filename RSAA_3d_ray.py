...
#Entrées : une norme, une distance minimu entre les inclusions, les lois de probabilité pour les rayons des inbclusions et pour chaque phase, les paramètres choisis pour ces lois, une fraction volumique pour chaque phase incluse dans l'espace ambiant initialement vide et un triplet pour le domaine spatial étudié
...
def eucl3D(a,b):return(np.sqrt((a[0]-b[0])**2+(a[1]-b[1])**2+(a[2]-b[2])**2))

def vol3D(r):return(max(1,4/3*np.pi*r**3))#cas discret : une boule de rayon nul, qui contient son centre, est de volume 1.
...
#En pratique : les centres sont distribués selon la loi uniforme de l'espace ambiant.
...
def RSAA_dist3D(geom,delta,l_ray,par,frac_vol,dim):
 ...
 for phi in range(0,n_phi):
  ...
  C_vol=True
  #Volume initial occupé par la phase i
  #Vol=0
  #loi de probabilité paramétrée suivie par le rayon : appliquer en par[phi]
  l=l_ray[phi]
  while C_vol:
   ...
   C_ent=True
   ##condition supplémentaire : boule incluse dans le domaine, cas des normes 1, 2 et infini. Vérification faite avant de parcourir liste_ph
   ...
   if C_ent:
    ...
    print(vol_inc[phi])
   ##Sortie de boucle : on compare le volume nouvellement occupé par la phase courante à celui prévu en entrée
   ##vol_inc[phi]=vol_inc[phi]+geom[1](ray)
   ##print(vol_inc[phi])
   ##print('factice')
...
#Exemple d'appel de la fonction RSAA_ray : géométrie euclidienne, 2+1 phases, rayons distribués suivant des lois normales, grille cubique de taille 100
